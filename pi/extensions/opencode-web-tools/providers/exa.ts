			...
			body: JSON.stringify({
				...
				params: {
					...
					arguments: {
						...
						contextMaxCharacters: clampContextMaxCharacters(input.contextMaxCharacters),
					},
				},
			}),
			signal,
...
function clampContextMaxCharacters(value: number | undefined): number {
	if (!Number.isFinite(value)) return DEFAULT_CONTEXT_MAX_CHARACTERS;
	const normalized = Math.round(value);
	return Math.max(200, Math.min(20_000, normalized));
}
