...
import type { SearchLivecrawl, SearchProvider } from "./providers/types.ts";
	...
	return {
		...
		parameters: Type.Object({
			...
			numResults: Type.Optional(
				Type.Number({
					description: "Alias for maxResults for compatibility with existing prompts and tooling.",
				}),
			),
			depth: Type.Optional(
			...
			type: Type.Optional(
				StringEnum([...SEARCH_DEPTHS], {
					description: "Alias for depth for compatibility with existing prompts and tooling.",
				}),
			),
			livecrawl: Type.Optional(
				StringEnum(["fallback", "preferred"] as const, {
					description: "Exa livecrawl mode. Defaults to fallback.",
				}),
			),
			contextMaxCharacters: Type.Optional(
				Type.Number({
					description: "Max context characters returned by the provider.",
				}),
			),
		}),
		...
		async execute(
			...
			const maxResults = clampMaxResults(params.maxResults ?? params.numResults ?? settings.search.defaultMaxResults);
			const depth = params.depth ?? params.type ?? settings.search.defaultDepth;
			...
			try {
				...
				const results = await provider.search({ query, maxResults, depth, livecrawl, contextMaxCharacters }, composed.signal);
		...
		renderCall(args: { query: string; depth?: SearchDepth; type?: SearchDepth; maxResults?: number; numResults?: number }, theme: any) {
			const depth = args.depth ?? args.type;
			const maxResults = args.maxResults ?? args.numResults;
