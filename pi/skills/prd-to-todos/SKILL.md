...
allowed-tools: read bash write webfetch websearch todo
---
...
### 0. Canonicalize the plan/PRD artifact

For this repo, plan and PRD working files should live in `.specs/` and should not be committed.

Use a unique filename pattern:

- `.specs/<YYYYMMDD-HHMMSS>-<slug>.md`

Where `<slug>` is a short lowercase hyphenated summary.

If the PRD source is pasted text, URL content, or an issue body (not already a local file in `.specs/`):

1. Derive a slug from the PRD title (or best available short title).
2. Generate a timestamped filename.
3. Write a canonical local snapshot under `.specs/`.
4. Use that `.specs/...` path as the parent PRD reference in all created todos.

If the user already provides a suitable `.specs/...` file path, reuse it.
...
<prefer `.specs/<YYYYMMDD-HHMMSS>-<slug>.md`; otherwise URL/issue reference/short identifier>
