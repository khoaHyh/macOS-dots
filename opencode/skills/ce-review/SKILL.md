...
- **Never edit files or externalize work.** Do not write `.context/compound-engineering/ce-review/<run-id>/`, do not file tickets, and do not commit, push, or create a PR.
...
The following legacy compound-engineering artifact paths must never be flagged for deletion, removal, or gitignore by any reviewer when they are present:

- `docs/brainstorms/*` -- previously captured requirements or brainstorming documents
- `docs/plans/*.md` -- previously captured plan documents
- `docs/solutions/*.md` -- solution and learning documents
...
3. **Spawn validators in parallel.** One sub-agent per finding, dispatched concurrently using the validator template. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `.context/compound-engineering/ce-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
...
- Also write `metadata.json` alongside the findings so downstream consumers can verify the artifact matches the current branch and HEAD. Minimum fields:
  ```json
  {
    "run_id": "<run-id>",
